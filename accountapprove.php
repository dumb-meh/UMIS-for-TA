...
if (isset($_GET["success"])) {
    if ($_GET["success"] == "approve") {
        echo '<div style="color: white; text-align: center; margin-top: 20px;">Account approved Successfully</div>';
    }

    if ($_GET["success"] == "delete") {
        echo '<div style="color: white; text-align: center; margin-top: 20px;">Account deleted Successfully</div>';
    }
}
...
<div class=card1>
    <table width=800px border='10' cellpadding='10' style='border:3px solid skyblue; border-collapse:collapse'>
        ...
        <tr>
            ...
            <th width=75px>Approve</th>
            <th width=75px>Delete</th>
        </tr>
        </thead>
        <tbody>
        <?php
        $sql = "SELECT * FROM users WHERE active='0' LIMIT $recordsPerPage OFFSET $offset";
        ...
        if ($resultCheck > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '</td>';
                echo '<td align="center">
                    <form action="approveaccount.php" method="post" onsubmit="return confirm(\'Are you sure you want to approve this?\');">
                        <input type="hidden" name="email" value="' . $row['usersEmail'] . '">
                        <button class="button login__submit" type="submit" name="submit"><span class="button__text">Approve</span></button>
                    </form>
                </td>';
